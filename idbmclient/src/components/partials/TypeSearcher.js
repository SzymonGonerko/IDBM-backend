...
export const TypeSearcher = (props) => {
  ...
  const { getSomeTitles, getSomeDirectors } = useSearch();
  ...
  useEffect(() => {
    if (timer == -2 && answers.length > 1) {
      if (director) {
        getSomeDirectors(answers).then((r) => {
          setHints(() => {
  ...
  useEffect(() => {
    ...
    const firstId = setInterval(() => setTimer((p) => p - 1), [500]);
    const secondId = setInterval(() => setSeparator((p) => (p == '|' ? '' : '|')), 500);
    return () => {
